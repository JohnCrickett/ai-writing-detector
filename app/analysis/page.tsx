      ...
      {segments.map((segment, idx) =>
        segment.type === 'highlight' ? (
          <span
            key={idx}
            style={{
              backgroundColor: segment.category?.includes('Vocabulary') ? '#fbbf24' : segment.category?.includes('Superficial') ? '#8b5cf6' : segment.category?.includes('Promotional') ? '#14b8a6' : '#ec4899',
              color: '#1e293b',
            }}
            ...
            <div className="mt-4 text-sm text-slate-600 dark:text-slate-400">
              ...
              <div className="flex flex-wrap gap-2">
                {Array.from(new Set(data.highlights.map((h) => h.category))).map((category) => {
                    const bgColor = category?.includes('Vocabulary') ? 'bg-amber-100 dark:bg-amber-900 text-amber-800 dark:text-amber-200' : category?.includes('Superficial') ? 'bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200' : category?.includes('Promotional') ? 'bg-teal-100 dark:bg-teal-900 text-teal-800 dark:text-teal-200' : 'bg-pink-100 dark:bg-pink-900 text-pink-800 dark:text-pink-200';
                  return (
                    <span
                      key={category}
            ...
            {data.patterns && data.patterns.length > 0 && (
              <>
                ...
                {data.patterns.map((pattern, idx) => {
                   const barColor = pattern.category === 'AI Vocabulary' ? '#fbbf24' : pattern.category === 'Superficial Analysis' ? '#8b5cf6' : pattern.category === 'Promotional Language' ? '#14b8a6' : '#ec4899';
                   
                   const labelColor = pattern.category === 'AI Vocabulary' ? 'text-amber-700 dark:text-amber-300' : pattern.category === 'Superficial Analysis' ? 'text-purple-700 dark:text-purple-300' : pattern.category === 'Promotional Language' ? 'text-teal-700 dark:text-teal-300' : 'text-pink-700 dark:text-pink-300';
