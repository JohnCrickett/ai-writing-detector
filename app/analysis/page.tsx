      ...
      {segments.map((segment, idx) =>
        segment.type === 'highlight' ? (
          <span
            key={idx}
            style={{
              backgroundColor: segment.category?.includes('Vocabulary') ? '#fbbf24' : segment.category?.includes('Superficial') ? '#8b5cf6' : segment.category?.includes('Promotional') ? '#14b8a6' : segment.category?.includes('Outline') ? '#ef4444' : segment.category?.includes('Negative Parallelism') ? '#f97316' : '#ec4899',
              color: '#1e293b',
            }}
            ...
            {data.patterns && data.patterns.length > 0 && (
              <>
                ...
                {data.patterns.map((pattern, idx) => {
                    const barColor = pattern.category === 'AI Vocabulary' ? '#fbbf24' : pattern.category === 'Superficial Analysis' ? '#8b5cf6' : pattern.category === 'Promotional Language' ? '#14b8a6' : pattern.category === 'Outline Conclusion Pattern' ? '#ef4444' : pattern.category === 'Negative Parallelism' ? '#f97316' : '#ec4899';
                    
                    const labelColor = pattern.category === 'AI Vocabulary' ? 'text-amber-700 dark:text-amber-300' : pattern.category === 'Superficial Analysis' ? 'text-purple-700 dark:text-purple-300' : pattern.category === 'Promotional Language' ? 'text-teal-700 dark:text-teal-300' : pattern.category === 'Outline Conclusion Pattern' ? 'text-red-700 dark:text-red-300' : pattern.category === 'Negative Parallelism' ? 'text-orange-700 dark:text-orange-300' : 'text-pink-700 dark:text-pink-300';
