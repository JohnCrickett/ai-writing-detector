            ...
            <div className="mt-4 text-sm text-slate-600 dark:text-slate-400">
              ...
              <div className="flex flex-wrap gap-2">
                {Array.from(new Set(data.highlights.map((h) => h.category))).map((category) => {
                   const bgColor = category?.includes('Vocabulary') ? 'bg-amber-100 dark:bg-amber-900 text-amber-800 dark:text-amber-200' : category?.includes('Superficial') ? 'bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200' : category?.includes('Promotional') ? 'bg-rose-100 dark:bg-rose-900 text-rose-800 dark:text-rose-200' : category?.includes('Outline') ? 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200' : category?.includes('Negative Parallelism') ? 'bg-orange-100 dark:bg-orange-900 text-orange-800 dark:text-orange-200' : category?.includes('Vague Attribution') ? 'bg-indigo-100 dark:bg-indigo-900 text-indigo-800 dark:text-indigo-200' : category?.includes('Overgeneralization') ? 'bg-cyan-100 dark:bg-cyan-900 text-cyan-800 dark:text-cyan-200' : category?.includes('Elegant Variation') ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' : 'bg-pink-100 dark:bg-pink-900 text-pink-800 dark:text-pink-200';
                  return (
                    <span
                      key={category}
            ...
            {data.patterns && data.patterns.length > 0 && (
              <>
                ...
                {data.patterns.map((pattern, idx) => {
                    const barColor = pattern.category === 'AI Vocabulary' ? '#fbbf24' : pattern.category === 'Superficial Analysis' ? '#8b5cf6' : pattern.category === 'Promotional Language' ? '#f43f5e' : pattern.category === 'Outline Conclusion Pattern' ? '#ef4444' : pattern.category === 'Negative Parallelism' ? '#f97316' : pattern.category === 'Vague Attributions' ? '#6366f1' : pattern.category === 'Overgeneralization' ? '#06b6d4' : pattern.category === 'Elegant Variation' ? '#10b981' : '#ec4899';
                    
                    const labelColor = pattern.category === 'AI Vocabulary' ? 'text-amber-700 dark:text-amber-300' : pattern.category === 'Superficial Analysis' ? 'text-purple-700 dark:text-purple-300' : pattern.category === 'Promotional Language' ? 'text-rose-700 dark:text-rose-300' : pattern.category === 'Outline Conclusion Pattern' ? 'text-red-700 dark:text-red-300' : pattern.category === 'Negative Parallelism' ? 'text-orange-700 dark:text-orange-300' : pattern.category === 'Vague Attributions' ? 'text-indigo-700 dark:text-indigo-300' : pattern.category === 'Overgeneralization' ? 'text-cyan-700 dark:text-cyan-300' : pattern.category === 'Elegant Variation' ? 'text-green-700 dark:text-green-300' : 'text-pink-700 dark:text-pink-300';
